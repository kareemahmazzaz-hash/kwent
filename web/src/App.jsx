...
{id:"c191",name:"Vrihedd Brigade Recruit (1)",faction:"scoiatael",power:4.0,row:"agile",cardType:"Basic",ability:null,abilityMeta:{},img:"Vrihedd Brigade Recruit1.png"},
{id:"c192",name:"Vrihedd Brigade Recruit (2)",faction:"scoiatael",power:4.0,row:"agile",cardType:"Basic",ability:null,abilityMeta:{},img:"Vrihedd Brigade Recruit2.png"},
{id:"c193",name:"Vrihedd Brigade Recruit (3)",faction:"scoiatael",power:4.0,row:"agile",cardType:"Basic",ability:null,abilityMeta:{},img:"Vrihedd Brigade Recruit3.png"},
...
// Ability-filter-row icon SVGs (DeckBuilder's Available/Chosen filter
// strips). Each is a single currentColor shape so it inherits the button's
// text color and flips correctly between the default and .active states,
// same as MusterIconSVG/BondIconSVG below (which the Muster and Bond
// filters reuse directly rather than getting their own icon here).

// Medic Icon: heart silhouette with a cross cut out as negative space via
// fill-rule evenodd (both subpaths in one path — no mask/id needed, so it's
// safe to render twice at once when Available and Chosen filters are both
// on screen). Source: SVG Repo "heart" (CC0).
function MedicFilterIcon() {
  return (
    <svg viewBox="0 0 512 512" className="ability-icon-svg" xmlns="http://www.w3.org/2000/svg">
      <path fill="currentColor" fillRule="evenodd" d="M458.2,35.8c-50.9-43-132.1-29-202.2,33.1-70.1-62-151.3-76-202.2-33.1-63.6,53.7-55.4,174.9,18.7,276,49.1,66.9,164.4,178.8,169.3,183.4,11.2,10.4,24.2,4.4,28.3,0,4.6-5,120.2-116.5,169.3-183.4,74.2-101,82.4-222.3,18.8-276z M344.2,276.4h-67.8v67.9c0,11.3-9.1,20.4-20.4,20.4-11.3,0-20.4-9.1-20.4-20.4v-67.9h-67.8c-11.3,0-20.4-9.1-20.4-20.4,0-11.3,9.1-20.4,20.4-20.4h67.8v-67.9c0-11.3,9.1-20.4,20.4-20.4,11.3,0,20.4,9.1,20.4,20.4v67.9h67.8c11.3,0,20.4,9.1,20.4,20.4,0,11.3-9.1,20.4-20.4,20.4z" />
    </svg>
  );
}

// Spy Icon: eye. Source: SVG Repo "eye" (CC0).
function SpyFilterIcon() {
  return (
    <svg viewBox="0 0 224.549 224.549" className="ability-icon-svg" xmlns="http://www.w3.org/2000/svg">
      <g fill="currentColor">
...
// Mardroeme Icon: mushroom cap silhouette, mirrored to match source
// orientation (mirror applied via a <g> transform + compensating translate
// rather than on the <svg> root, since a root-level transform pushes the
// shape outside the viewBox in most renderers). Source: SVG Repo (CC0).
function MardroemeFilterIcon() {
  return (
    <svg viewBox="0 0 512 512" className="ability-icon-svg" xmlns="http://www.w3.org/2000/svg">
...
// Avenger Icon: three-eyed goat head. Re-traced from a photo-realistic
// source (which was an 11.8MB, ~3,470-path autotrace) into a single clean
// currentColor path via threshold + potrace, ~15KB.
function AvengerFilterIcon() {
  return (
    <svg viewBox="0 0 1024 1024" className="ability-icon-svg" xmlns="http://www.w3.org/2000/svg">
 1.3 2.6 4.1 3.5 6.2m58.6-3.6c2.6 2.6 4.8 5.3 4.8 6 0 1.4-5 4.3-7.5 4.3-1.1 0-1.5-1.1-1.5-4.9 0-2.7-.4-6.1-1-7.5-1.4-3.7-.2-3.2 5.2 2.1m-10.8 3.3c1.7 4.6 1.7 5.3.4 6.7-.9.8-3.3 1.7-5.6 2-3.3.4-4.2.2-5.1-1.5s-.5-3.1 2.6-8c2.4-3.7 4.2-5.6 4.8-5.1.6.4 1.9 3.1 2.9 5.9m-31.9 0c1.6 3.3 3.2 6 3.7 6 .4 0 2.2-2.7 3.8-6.1 2.6-5.1 3.3-5.9 4.6-4.8 2.7 2.2 6.6 9.8 6.2 12.1-.3 2.3-.5 2.3-14.3 2.3h-14l-.3-3.2c-.2-2.4.5-4.5 2.9-7.8 1.8-2.5 3.5-4.5 3.9-4.5.3 0 1.9 2.7 3.5 6" />
    </svg>
  );
}

// Berserker Icon: bear/wolf head with fine detail lines cut in via an SVG
// mask. Unlike the other filter icons this one genuinely needs a mask (the
// detail lines aren't a simple single-hole shape), so it uses useId for a
// unique mask id — the Available and Chosen filter rows can both have this
// icon on screen at once, and a hardcoded id would collide between them.
function BerserkerFilterIcon() {
  const maskId = useId();
  return (
    <svg viewBox="0 0 2048 2048" className="ability-icon-svg" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <mask id={maskId} maskContentUnits="userSpaceOnUse">
          <rect width="2048" height="2048" fill="#fff" />
          <path d="M987.932 1051.42c12.268-1.57 34.838-1.3 47.398.21 64.96 7.77 117.86 15.35 109.45 93.39-36.42-22.18-89.18-23.78-131.04-24.14-32.65.25-73.734 1.24-104.514 13.22-17.999 7-20.272 18.57-26.308 33.91-7.291-35.42-13.511-77.06 25.147-97.48 24.222-12.8 52.834-16.81 79.867-19.11m278.658-335.219c28.43-1.849 43.77 10.992 62.38 30.357-21.34 9.57-46.62 21.324-67.16 32.277 2.66 52.766-22.84 92.838-81.77 84.342-14.14-2.038-26.71-9.446-38.72-16.897-26.25-20.301-37.83-44.27-51.87-72.938 17.07 18.303 39.32 42.065 66.24 28.029 44.57-23.241 58.64-75.635 110.9-85.17m-264.91-301.769c10.44-.367 20.89-.348 31.32.059 43.88 1.668 64.92 13.794 93.97 45.195-25.76-11.857-55.11-18.881-83.21-21.919l-10.15-.792c3.7 7.819 6.2 13.313 9.1 21.415-26.44-13.701-86.498-28.218-117.3-33.403 24.162-6.797 51.242-9.344 76.27-10.555m-56.113 749.708c37.113-.17 39.178 45.38 38.659 72.34-3.712-4.14-6.85-6.76-11.011-10.38-17.033-11.57-29.693-13.72-49.592-19.15 2.866-10.78 2.19-14.44 1.481-25.48 4.841-10.88 9.183-14.13 20.463-17.33m135.663 7.99c25.38 1.55 28.13 20.44 28.29 41.57-19.27.15-29.86-.6-46.79 10.16-7.26 5.24-10.93 8.76-17.31 15.06 2.62-23.28 9.75-59.23 35.81-66.79" fill="#000" />
        </mask>
      </defs>
      <g mask={`url(#${maskId})`} fill="currentColor">
        <path d="M471.151 946.674c2.227 3.322 9.104 25.107 10.985 30.377a272 272 0 0 0 14.076 32.519c36.449 69.39 100.103 120.23 166.352 159.4a1253 1253 0 0 0 46.21 25.6c14.124 7.48 35.424 17.21 47.068 27.48 8.627 8.06 13.139 21.1 19.391 30.4 15.183 22.57 30.811 59.08 48.533 78.42l31.249-4.13c9.229 37.74 32.756 56.92 70.668 64.69 55.154 11.31 122.537 21.73 177.207 3.89 21.2-6.92 49.13-33.39 58.09-53.21 9.33-.25 20.1.05 29.07-2.61 26.63-7.88 73.81-94.32 88.32-120.5 5.49-2.77 11.8-5.48 17.48-7.93 71.37-30.79 141.6-74.68 192.86-133.92 30.65-35.41 60.73-79.626 83.39-120.803 9.19 15.859 19.7 40.727 26.71 57.703 33.37 82.52 56.61 174.33 48.37 263.97-10.2 111.02-76.7 185.63-158.63 252.66-72.28 59.96-156.45 109.93-235.42 160.44-19.97 12.78-41.84 26.88-62.65 38.06 6.26-20.58 17.54-48.17 25.25-68.74-64.45 45.7-140.77 104.26-191.68 164.52-47.995-58.22-108.274-112.72-165.185-162.11 1.249 20.87 2.866 37.72 5.473 58.56-51.726-23.96-112.366-90.72-144.377-137.38 1.885 25.05 4.065 42.31 7.94 67.24-33.82-14.65-82.312-51.7-111.547-74.78-99.001-78.15-198.812-174.64-236.017-298.77-4.988-16.65-7.527-36.06-5.239-53.43 10.793-72.92 42.51-146.25 68.955-214.89 4.992-13 40.597-52.275 27.098-62.726" />
        <path d="M1019.95 1319.44c34.37 1.3 81.33 11.55 114.61 20.88-14.47 16.03-22.06 25.38-45.45 25.05-29.52-.43-59.1-2.37-88.58-4.15-30.319-1.13-61.825-1.33-92.236-1.77-10.081-.15-19.926-10-25.987-17.81a2375 2375 0 0 1 84.148-15.16c17.25-2.81 36.165-6.08 53.495-7.04" />
      </g>
      <path fill="currentColor" d="M815.501 785.404c11.737 6.285 43.977 26.752 54.721 34.638l-7.786 7.171c-12.939 8.263-29.324 8.978-39.453-4.003-8.802-11.28-8.812-24.35-7.482-37.806m401.539 12.566c1.5-.073.59-.148 2.66.697 8.11 20.738-17.09 51.223-44.57 36.104-6.68-4.946-7.89-7.705-11.54-14.872 13.87-6.594 37.7-15.095 53.45-21.929" />
    </svg>
  );
}

const ABILITY_FILTERS = [
  { key: "muster", label: "Muster", icon: MusterIconSVG },
  { key: "medic", label: "Medic", icon: MedicFilterIcon },
  { key: "decoy", label: "Decoy", icon: DecoyFilterIcon },
  { key: "spy", label: "Spy", icon: SpyFilterIcon },
  { key: "tightBond", label: "Bond", icon: BondIconSVG },
  { key: "moraleBoost", label: "Morale", icon: MoraleFilterIcon },
  { key: "horn", label: "Horn", icon: HornFilterIcon },
  { key: "weather", label: "Weather", symbol: "\u2601" },
  { key: "clearWeather", label: "Clear Weather", symbol: "\u2600" },
  { key: "scorch", label: "Scorch", icon: ScorchFilterIcon, match: ["scorchGlobal", "scorchRow", "scorchRowThreshold"] },
  { key: "berserker", label: "Berserker", icon: BerserkerFilterIcon },
  { key: "mardroeme", label: "Mardroeme", icon: MardroemeFilterIcon },
  { key: "summonAvenger", label: "Avenger", icon: AvengerFilterIcon },
];
  ...
  return (
    <div className="leader-carousel">
      ...
      <button type="button" className="leader-icon-btn" onClick={openCarousel}>
        ...
        <span className="leader-icon-hint desktop-only-el">{chosen ? "Change leader" : "Choose leader"}</span>
      </button>

      {shown && !expanded && (
        <div className="leader-ability-box desktop-only-el">
          ...
          <div className="leader-zoom-content" onClick={(e) => e.stopPropagation()}>
            ...
            {shown && (
              <div className="leader-ability-box leader-zoom-ability-box">
                <strong className="desktop-only-el">{shown.name}</strong>
                <p>{abilityDescriptionFor(shown)}</p>
              </div>
            )}
          </div>
  ...
  return (
    <div className="ability-filter-row">
      {filters.map((f) => (
        ...
        >
          {f.icon ? <f.icon /> : <span className="ability-filter-symbol">{f.symbol}</span>}
        </button>
      ))}
      {active && (
      ...
      <div className="deckbuilder-header">
        ...
        {onSaveDeck && (
          <div className="saved-decks-row">
            ...
            {onRandomize && (
              <button type="button" className="btn btn-sm random-deck-btn mobile-only-el" onClick={onRandomize}>
                🎲 Random deck
              </button>
            )}
          </div>
      ...
      <div className="db-columns">
        ...
        <div className="db-col db-col-leader">
          ...
          <div className="deck-count">
            <span className="deck-count-line desktop-only-el">Selected: <strong>{count}</strong> cards — <strong>{unitCount}</strong> / {DECK_SIZE} minimum unit cards</span>
            <span className="deck-count-line desktop-only-el"><strong>{specialCount}</strong> / {MAX_SPECIAL_CARDS} max special cards</span>
            <span className="deck-count-line mobile-only-el">Unit: <strong>{unitCount}</strong>/{DECK_SIZE}</span>
            <span className="deck-count-line mobile-only-el">Special: <strong>{specialCount}</strong>/{MAX_SPECIAL_CARDS}</span>
            {onRandomize && (
              <button type="button" className="btn btn-sm random-deck-btn desktop-only-el" onClick={onRandomize}>
                🎲 Random deck
              </button>
            )}
          </div>
...
/* Desktop/mobile-only toggle pairs — used where the same control needs
   different placement, text, or presence on phones vs desktop (deck-count
   wording, confirm-button location, leader name/hint). Both elements are
   always rendered; only one is ever visible, flipped by the same
   (max-width: 520px), (max-height: 480px) breakpoint used everywhere else
   in the deck builder's mobile styling. */
.mobile-only-el { display: none; }
@media (max-width: 520px), (max-height: 480px) {
  .desktop-only-el { display: none !important; }
  .mobile-only-el { display: revert; }
}
...
@media (max-width: 520px), (max-height: 480px) {
  ...
  /* Leader zoom overlay: use the dynamic viewport unit so mobile browser
     ...
     content behind the fixed overlay could show through, and let the
     ability text scroll instead of getting clipped/overlapping the strip.
     Cards are sized off the track's HEIGHT (not a width percentage) so on
...
@media (max-width: 520px), (max-height: 480px) {
  ...
  .mode-grid { gap: 2.5vh; margin: 0; flex: 0 1 auto; overflow: hidden; }
  .mode-card { padding: 3vh 5%; }
  .mode-title { font-size: clamp(1rem, 3.4vh, 1.3rem); margin-bottom: 0.6vh; }
  .mode-desc { font-size: clamp(0.82rem, 2.6vh, 1rem); }
}
