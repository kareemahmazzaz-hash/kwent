...
.screen.home { min-height: unset; height: 100vh; height: 100dvh; overflow: hidden; display: flex; flex-direction: column; justify-content: center; box-sizing: border-box; padding: 3% 4%; }
...
/* ---- Home ---- */
.home-hero { text-align: center; padding: 0 0 2vh; flex: 0 0 auto; }
.eyebrow { font-family: var(--font-mono); font-size: clamp(0.55rem, 1.8vh, 0.7rem); letter-spacing: 0.18em; color: var(--gold-dim); }
.home-hero h1 { font-family: var(--font-display); font-size: clamp(1.5rem, 6vh, 2.4rem); margin: 1vh 0; color: var(--gold); text-shadow: 0 2px 12px rgba(201,162,75,0.25); }
.home-hero p { color: var(--muted); max-width: 480px; margin: 0 auto; }
.mode-grid { display: grid; gap: 1.6vh; margin: 0; flex: 0 1 auto; overflow: hidden; }
.mode-card { background: linear-gradient(180deg, var(--bg-panel-2), var(--bg-panel)); border: 1px solid var(--line); border-radius: 10px; padding: 1.8vh 4.5%; text-align: left; cursor: pointer; color: var(--parchment); transition: border-color .15s, transform .15s; }
...
.mode-title { display: block; font-family: var(--font-display); font-size: clamp(0.85rem, 2.6vh, 1.05rem); color: var(--gold); margin-bottom: 0.5vh; }
.mode-desc { display: block; font-size: clamp(0.72rem, 2.1vh, 0.88rem); color: var(--muted); }
